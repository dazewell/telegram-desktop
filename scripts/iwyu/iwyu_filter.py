...
def filter_iwyu_output(
    ...
    `should remove` lines that point at `platform/<X>/...` headers are
    always dropped: those are the platform-dispatch entries IWYU misreads
    on a single-platform analysis (the inline definition lives in the
    not-currently-active branch of the `#ifdef`, and removing the include
    breaks the link). The block as a whole is kept so other valid removes
    survive.
...
def _drop_platform_dispatch_removes(block: list[str]) -> tuple[list[str], int]:
    """Inside a block, drop `- #include "platform/<X>/..."` removal lines.

    These almost always sit inside an `#ifdef Q_OS_<X>` in the source and
    are invisible to IWYU's per-platform analysis. The Linux build will
    suggest dropping the Linux include because IWYU only sees the Linux
    branch as inactive in the single TU it analysed (or vice versa);
    removing the include breaks any inline definition that lives there.
    """
    out: list[str] = []
    dropped = 0
    in_remove = False
