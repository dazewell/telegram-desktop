...
def _detect_compiler_system_includes(build_dir: Path) -> list[str]:
    """Run the configured compiler with `-E -v -xc++ /dev/null` and parse
    the system include search paths it would use.

    On Linux centos_env the project compiles with gcc-toolset-15, whose
    libstdc++ lives at /opt/rh/gcc-toolset-15/root/usr/include/c++/15.
    IWYU's bundled clang has no idea about that and falls back to system
    gcc's libstdc++ (gcc 8 on Rocky 8) — where C++20 headers like
    `<compare>` and `std::strong_ordering` simply do not exist, so every
    file that uses them fails analysis with no IWYU output.

    Pinning each path as `-isystem` makes IWYU see exactly what the build
    sees, regardless of which gcc layout the host has.
    """
    if platform.system() == "Darwin":
        return []
    compiler = _read_cmake_compiler(build_dir)
    ...
    for line in text.splitlines():
        ...
        if s.startswith("End of search list"):
            break
        if in_block and s:
            # Trim trailing framework markers ("(framework directory)").
            paths.append(s.split(" (")[0])
    return paths


# Compile flags emitted by gcc 13+ that clang/IWYU does not understand.
# `-fhardened` causes a fatal `unknown argument:` error which silently
# kills the IWYU run on every TU that has it; the warning flags only
# produce noise but are dropped for log cleanliness.
...
def _strip_pch_from_command(tokens: list[str]) -> list[str]:
    """Remove `-Xclang -include-pch <path>` triples and any gcc-only flags
    that confuse the clang inside IWYU.
